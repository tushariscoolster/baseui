...
We have built in functionality based on the reccomendations for dialogs and modals in [WAI-ARIA Authoring Practices 1.1](https://www.w3.org/TR/wai-aria-practices/#dialog_modal).
...
In these cases, it is reccomended by WAI-ARIA to avoid skipping content and focusing on a normally non-focusable element at the top of the content (such as an `h1` or `p` tag).
